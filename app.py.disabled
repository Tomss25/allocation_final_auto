    ...
    if st.button("🚀 GENERA SERIE STORICHE", type="primary", use_container_width=True):
        with st.spinner("Acquisizione Dati in corso..."):
            ...
            if input_type == "API (Ticker/ISIN)" and tickers_input:
                ...
                try:
                    df_temp = pd.read_csv(uploaded_file, sep=';', decimal=',', index_col=0, parse_dates=True) if uploaded_file.name.endswith('.csv') else pd.read_excel(uploaded_file, index_col=0, parse_dates=True)
