...
def optimize_portfolio_3(returns, annual_factor, min_weight=0.0):
    ...
    res = minimize(objective, [1./n_assets]*n_assets, method='SLSQP', bounds=[(min_weight, 1)]*n_assets, constraints=({'type': 'eq', 'fun': lambda x: np.sum(x) - 1}), method='SLSQP')
    return res.x if res.success else None
...
if page == "Nota Metodologica":
    ...
    * **Stazionarietà:** I modelli classici assumono implicitamente che i rendimenti pasts siano stimatori non distorti del futuro.
    * **Distribuzione:** La varianza assume normalità dei rendimenti, assioma fallace. Il modulo CVaR e le proiezioni stocastiche rilassano questa assunzione.
    ...
    if mu_strat is None:
        ...
        with tab1:
            ...
            # Matrice di Correlazione (Rimpicciolita come richiesto)
            fig, ax = plt.subplots(figsize=(4, 2.5)) # Ridimensionamento della figura matplotlib
            sns.heatmap(df.pct_change().corr(), annot=True, cmap="RdYlGn", fmt=".2f", ax=ax, annot_kws={"size": 7}) # Font annotazioni ridotto
            ax.tick_params(labelsize=7) # Font etichette assi ridotto
            
            # Visualizzazione rimpicciolita tramite colonne (OBBLIGO: Restringimento container visivo)
